...
contract('vesting.js', function(accounts) {
  ...
  it('Finalize as the pre-sale participant', async () => {
	...
	it('Release tokens - month by month', async	 () => {
		...
		await tokenVestingInstance.release({from:constants.PARTICIPANT_PRESALE});
		let users = await tokenVestingInstance.users(constants.PARTICIPANT_PRESALE);
		let balanceOf = Number(await erc20Instance.balanceOf(constants.PARTICIPANT_PRESALE));
	...
	it('Release tokens - cliff then 5 months in', async	 () => {
		let user = await tokenVestingInstance.users(constants.PARTICIPANT_PRESALE);
		let totalRequired = Number(user[7]);
		increaseTime(YEAR_EPOCH);
		assert.equal(await tokenVestingInstance.fullDurationMet(), true);

		await tokenVestingInstance.release({from:constants.PARTICIPANT_PRESALE});
		assert.equal(Number(await erc20Instance.balanceOf(constants.PARTICIPANT_PRESALE)), totalRequired);
		let presaleBalance = Number(await erc20Instance.allowance(auctionInstance.address, tokenVestingInstance.address));
