...
contract Bridge {
  ...
  function registerToken(ERC20BurnableAndMintable _token) public {
  ...
  function startExit(bytes32[] _proof, uint256 _oindex) public {
    ...
    uint256 exitable_at = Math.max(periods[_proof[0]].timestamp + (2 * exitDuration), block.timestamp + exitDuration);
  ...
  /*
    CREDIT: parseclabs - parseclabs.org
  */
